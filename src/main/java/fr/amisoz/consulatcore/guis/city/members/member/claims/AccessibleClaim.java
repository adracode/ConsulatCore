...
public class AccessibleClaim extends GuiListener<UUID> {
    ...
    public AccessibleClaim(){
        super(6);
        setTemplate("Claims accessible",
                getItem("§7Accès", GIVE_ALL_SLOT, Material.END_CRYSTAL, "§7Donner l'accès à tous les claims"),
                getItem("§7Accès", REMOVE_ALL_SLOT, Material.BARRIER, "§7Retirer l'accès de tous les claims"))
    ...
    @Override
    public void onOpen(GuiOpenEvent<UUID> event){
    }
    
    @Override
    public void onClose(GuiCloseEvent<UUID> event){
    }
